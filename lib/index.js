...
//Public functions
		...
		this.request('REPORT', path, body, function (err, res) {
			...
			tools.parseXML( res.body, function ( err, data ) {
				...
				async.map(data.response, function(internal, callback){
					...
					newObj['addressbook-data'] = tools.parseEventData(newObj['addressbook-data']);	
	...
	Sync.prototype.modifyAddressbook = function ( info, addressbook, callback ) {
		...
		this.request( 'PROPPATCH', addressbook, body, function (err, res) {
			console.log(err);
		});
